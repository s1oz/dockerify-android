...
- Image: `s1oz/dockerify-android` (Docker Hub) and `ghcr.io/s1oz/dockerify-android`
...
4. Repo **Settings → Secrets and variables → Actions** add:
   - `DOCKERHUB_USERNAME` (e.g. `s1oz`)
   - `DOCKERHUB_TOKEN` (Docker Hub access token)
5. Packages → `dockerify-android` → optional: set GHCR visibility **Public**.

## Pull

国内建议 Docker Hub（GHCR 经常很慢）：

```bash
docker pull s1oz/dockerify-android:latest
```

备用：`ghcr.io/s1oz/dockerify-android:latest`
...
4. CA Auto Update 勾选该容器，跟 `s1oz/dockerify-android:latest`
